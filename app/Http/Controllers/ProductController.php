        ...
        if ($images) {
            ...
            foreach ($images as $key => $image) {
                ...
                if (!config('database.connections.saleprosaas_landlord')) {
                    ...
                    // Use Image intervention with public_path() to save images in different sizes
                    $img_lg = Image::make(public_path('images/product/' . $imageName))
                                   ->fit(500, 500)
                                   ->save(public_path('images/product/large/' . $imageName), 90);
                    $img_md = Image::make(public_path('images/product/' . $imageName))
                                   ->fit(250, 250)
                                   ->save(public_path('images/product/medium/' . $imageName), 100);
                    $img_sm = Image::make(public_path('images/product/' . $imageName))
                                   ->fit(100, 100)
                                   ->save(public_path('images/product/small/' . $imageName), 100);
                } else {
                    $imageName = $this->getTenantId() . '_' . $imageName . '.' . $ext;
                    $image->move(public_path('images/product'), $imageName);

                    // Save resized images for multi-tenant setup
                    $img_lg = Image::make(public_path('images/product/' . $imageName))
                                   ->fit(500, 500)
                                   ->save(public_path('images/product/large/' . $imageName), 90);
                    $img_md = Image::make(public_path('images/product/' . $imageName))
